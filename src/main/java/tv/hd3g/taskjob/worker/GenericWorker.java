 ...
 */
@FunctionalInterface
public interface GenericWorker<T> {
	
	/**
	 * @param referer, job to process
	 * @param context, the deserialized object from referer context
	 * @param broker, broker to use for manipulate job status
	 * @param shouldStopProcessing if should stop processing now
	 */
	public void process(Job referer, T context, Broker broker, Supplier<Boolean> shouldStopProcessing) throws Throwable;
